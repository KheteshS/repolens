  ...
  if (node.type === "file") {
    ...
    return (
      ...
      >
        ...
        <span className={node.language ? LANG_COLORS[node.language] ?? "text-slate-700" : "text-slate-700"}>{node.name}</span>
      </button>
    );
  }
  ...
  return (
    <div>
      ...
      {open && node.children?.map((child) => (
        <Node key={child.path} node={child} depth={depth + 1} onFileClick={onFileClick} selectedPath={selectedPath} />
      ))}
    </div>
  );
}

function FileDetailPanel({ node, onClose }: { node: FileNode; onClose: () => void }) {
  return (
    <div className="absolute right-0 top-0 h-full w-72 bg-white border-l border-slate-200 shadow-xl p-4 overflow-y-auto z-10">
      ...
      <div className="flex flex-col gap-3">
        <div>
          ...
          <p className="text-sm font-medium text-slate-800 mt-0.5">{node.name}</p>
        </div>

        <div>
          <span className="text-[10px] text-slate-500 uppercase tracking-wider">Path</span>
          <p className="text-xs font-mono text-slate-600 mt-0.5 break-all">{node.path}</p>
        </div>

        {node.language && (
          <div>
            <span className="text-[10px] text-slate-500 uppercase tracking-wider">Language</span>
            <p className="text-sm text-slate-700 mt-0.5 capitalize">{node.language}</p>
          </div>
        )}

        <div>
          <span className="text-[10px] text-slate-500 uppercase tracking-wider">Type</span>
          <p className="text-sm text-slate-700 mt-0.5 capitalize">{node.type}</p>
        </div>
      </div>
    </div>
  );
...
export default function FileTree({ node }: { node: FileNode }) {
  ...
  return (
    <div className="relative font-mono text-xs leading-6">
      <Node node={node} depth={0} onFileClick={setSelectedFile} selectedPath={selectedFile?.path ?? null} />
      {selectedFile && (
        <FileDetailPanel node={selectedFile} onClose={() => setSelectedFile(null)} />
      )}
    </div>
  );
}
